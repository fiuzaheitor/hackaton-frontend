  ...
  const renderDays = () => {
    ...
    for (let day = 1; day <= days; day++) {
      ...
      if (filter === "Maternidade") {
        ...
        dataVaccinesByVaccineCard?.vaccinesByVaccineCard?.map(
          (vaccine: any) => {
            const vaccineDate = new Date(vaccine.applicationDate);
            const dayInMillis = 24 * 60 * 60 * 1000;
            const birthDate = new Date(dataUserKids.kidsByMom[0].birthDate);
            const vaccineStartDate = new Date(
              vaccineDate.getTime() - 4 * dayInMillis,
            );
            const vaccineEndDate = new Date(
              vaccineDate.getTime() + 3 * dayInMillis,
            );
            if (
              date >= vaccineStartDate &&
              date <= vaccineEndDate &&
              date >= birthDate
            ) {
              isEventDay = true;
