...
import {
  ...
  G_KIDS,
  G_KIDS_BY_MOM,
  G_VACCINES_BY_VACCINE_CARD,
  G_GESTATION,
  G_GESTATIONS,
  G_GESTATIONS_BY_USER,
  G_CONSULTATION,
  G_CONSULTATIONS,
  G_CONSULTATIONS_BY_GESTATION,
  G_VACCINE_CARD,
  G_VACCINE_CARDS,
...
export const useGetUsers = () => {
  const { data, loading, error } = useQuery(G_USERS, {
    fetchPolicy: "cache-and-network",
  });
...
export const useGetKids = () => {
  const { data, loading, error } = useQuery(G_KIDS, {
    fetchPolicy: "cache-and-network",
  });
...
export const useGetGestations = () => {
  const { data, loading, error } = useQuery(G_GESTATIONS, {
    fetchPolicy: "cache-and-network",
  });
...
export const useGetConsultations = () => {
  const { data, loading, error } = useQuery(G_CONSULTATIONS, {
    fetchPolicy: "cache-and-network",
  });
...
export const useGetVaccineCards = () => {
  const { data, loading, error } = useQuery(G_VACCINE_CARDS, {
    fetchPolicy: "cache-and-network",
  });
...
export const useGetVaccines = () => {
  const { data, loading, error } = useQuery(G_VACCINES, {
    fetchPolicy: "cache-and-network",
  });
